...
parser.add_argument("--data_dir",
                    default="data/attacked",
                    help="The dataset path.")
parser.add_argument("--model_dir",
                    default="models",
                    help="Path where to save models.")
parser.add_argument("--is_zero",
                    default=False,
                    type=int,
...
if __name__ == "__main__":
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(args.samples):
            ...
            input = prepare_data_for_classification(input, IMAGE_SIZE,
                                                    N_CHANELS)
            N += input.shape[0]
        ...
        diff = org_labels - np.reshape(pred_labels.argmax(1),
                                       (args.samples, -1))
        diff[diff != 0] = 1
        total_error = 100 * np.sum(diff) / N

        print('datatype = %s, attack_type = %s: \t error = %0.2f' %
              (args.type, args.attack_type, total_error))
